//! Core reactivity system for `wayglance`.
//!
//! This module enables dynamic UI updates after widgets are built, by providing:
//! - **[`state`]**: Reactive data bindings (`wayglance.state()`).
//! - **[`signals`]**: A global pub-sub event bus (`wayglance.emitSignal()`, `wayglance.onSignal()`).
//! - **[`timer`]**: GLib-backed asynchronous intervals (`wayglance.setInterval()`).
//!
